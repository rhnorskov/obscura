...
fn inline_classic_script_runs() {
    let server = TestServer::start(|url| match url {
        "/" => Some((
            ...
            "<!doctype html><html><body>\
                <div id=out></div>\
                <script>document.getElementById('out').textContent = 'classic-inline-ok';</script>\
                </body></html>"
                .into(),
        )),
        _ => None,
    ...
    rt().block_on(async {
        ...
        assert_eq!(v, Value::String("classic-inline-ok".into()));
    });
}

#[test]
fn inline_module_script_runs() {
    let server = TestServer::start(|url| match url {
        "/" => Some((
            200,
            "text/html",
            "<!doctype html><html><body>\
                <div id=out></div>\
                <script type=\"module\">document.getElementById('out').textContent = 'module-inline-ok';</script>\
                </body></html>"
                .into(),
        )),
        _ => None,
    ...
    rt().block_on(async {
        ...
        page.navigate_with_wait(&server.url("/"), WaitUntil::Load)
            ...
            .unwrap();
        let v = page.evaluate("document.getElementById('out').textContent");
        assert_eq!(v, Value::String("module-inline-ok".into()));
    });
}

#[test]
fn external_classic_script_runs() {
    let server = TestServer::start(|url| match url {
        "/" => Some((
            200,
            "text/html",
        ...
        "/x.js" => Some((
            ...
            "document.getElementById('out').textContent = 'classic-external-ok';".into(),
        )),
        _ => None,
    });
    ...
    rt().block_on(async {
        ...
        page.navigate_with_wait(&server.url("/"), WaitUntil::Load)
            ...
            .unwrap();
        let v = page.evaluate("document.getElementById('out').textContent");
        assert_eq!(v, Value::String("classic-external-ok".into()));
    });
}

#[test]
fn external_module_script_runs() {
    let server = TestServer::start(|url| match url {
        "/" => Some((
            200,
            "text/html",
        ...
        "/x.js" => Some((
            200,
            "application/javascript",
            "document.getElementById('out').textContent = 'module-external-ok';".into(),
        )),
        _ => None,
    });
    ...
    rt().block_on(async {
        ...
        page.navigate_with_wait(&server.url("/"), WaitUntil::Load)
            ...
            .unwrap();
        let v = page.evaluate("document.getElementById('out').textContent");
        assert_eq!(v, Value::String("module-external-ok".into()));
    });
}

#[test]
fn external_module_with_relative_import_runs() {
    let server = TestServer::start(|url| match url {
        "/" => Some((
            200,
            "text/html",
        ...
        "/dep.js" => Some((
            200,
            "application/javascript",
            "export function greet() { return 'module-relative-import-ok'; }".into(),
        )),
        _ => None,
    });
    ...
    rt().block_on(async {
        ...
        page.navigate_with_wait(&server.url("/"), WaitUntil::Load)
            ...
            .unwrap();
        let v = page.evaluate("document.getElementById('out').textContent");
        assert_eq!(v, Value::String("module-relative-import-ok".into()));
    });
}
